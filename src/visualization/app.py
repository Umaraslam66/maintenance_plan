...
def parse_date(date_str):
    ...
    try:
        if isinstance(date_str, str):
            # First, try to handle datetime strings with time components
            if " " in date_str:  # Contains space, likely datetime format
                # Try parsing as datetime format first
                try:
            ...
            return datetime.strptime(date_str, "%Y-%m-%d")  # Standard format
...
# Network Visualization page
    ...
    if st.session_state.network_data is None:
        ...
        with map_tab:
            ...
            folium_map = folium.Map(location=[62, 15], zoom_start=5)
            ...
            if show_maintenance and st.session_state.maintenance_data:
                for maintenance_item  in st.session_state.maintenance_data:
                    start_date = parse_date(maintenance_item.get('start_date'))
                    if start_date:
                        end_date = start_date + timedelta(days=maintenance_item.get('duration_days', 0))
                        if start_date.date() <= maintenance_date <= end_date.date():
                            track_id = maintenance_item.get('track_id')
                            if track_id not in maintenance_on_link:
                                maintenance_on_link[track_id] = []
                            maintenance_on_link[track_id].append(maintenance_item)
                ...
                if from_node and to_node and from_node.get('lat') and from_node.get('lon') and \
                   to_node.get('lat') and to_node.get('lon'):
                    ...
                    if is_detour:
                        ...
                        dash_array = None
                    print(f"Type of m before adding PolyLine: {type(folium_map)}")
                    ...
                    try:
                        ...
                        # Silently continue if map element can't be added
                        pass
            ...
            # Replace the legend section with a simple Streamlit-side legend
            if show_maintenance and maintenance_on_link:
                # Instead of adding to the folium map's HTML, create a sidebar legend in Streamlit
                with st.sidebar:
                    st.markdown("### Map Legend")
                    st.markdown("* <span style='color:blue'>■</span> High Capacity (>10)", unsafe_allow_html=True)
                    st.markdown("* <span style='color:green'>■</span> Medium Capacity (7-10)", unsafe_allow_html=True)
                    st.markdown("* <span style='color:orange'>■</span> Low Capacity (4-7)", unsafe_allow_html=True)
                    st.markdown("* <span style='color:red'>■</span> Very Low Capacity/Maintenance", unsafe_allow_html=True)
                    
                    if selected_detour != 'None':
                        st.markdown("* <span style='color:purple'>■</span> Original Link", unsafe_allow_html=True)
                        st.markdown("* <span style='color:green'>---</span> Detour Route", unsafe_allow_html=True)
    ...
    if st.session_state.maintenance_data is None:
        ...
        if start_date or end_date:
            filtered_measures = [
                ...
                if (
                    not start_date or parse_date(m.get('start_date', '')).date() >= start_date
                ) and (
                    not end_date or parse_date(m.get('start_date', '')).date() <= end_date
                )
            ]
                ...
                if st.session_state.network_data:
                    ...
                    for link in st.session_state.network_data['links']:
                        ...
                        to_node.get('lat') and to_node.get('lon'):
                            ...
                            # Add the link as a polyline with try-except
                            try:
                                folium.PolyLine(
                                    locations=[[from_node['lat'], from_node['lon']], [to_node['lat'], to_node['lon']]],
                                    color=color,
                    ...
                    if hasattr(root, 'html') and hasattr(root.html, 'add_child'):
                        ...
                        folium_map.get_root().get_name()  # No-op to prevent error
